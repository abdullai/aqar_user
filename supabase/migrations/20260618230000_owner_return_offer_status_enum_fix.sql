...
-- السبب: coalesce(status, '') على عمود enum يحوّل '' إلى offer_status ويفشل.
-- =============================================================================
...
BEGIN
  ...
  -- علّم العروض النشطة/المقبولة كـ withdrawn (لا تستخدم coalesce(status,'') على enum)
  UPDATE public.listing_offers
   ...
   WHERE request_id = p_request_id
     AND lower(status::text) IN (
       'submitted', 'pending', 'expired', 'owner_accepted', 'selected'
     );
    ...
    UPDATE public.properties p
     ...
     WHERE p.id = v_req.preview_property_id
       ...
       AND lower(trim(coalesce(p.workflow_stage, ''))) IN (
         ...
         'marketer_selected', 'published', 'reserved'
       );
  END IF;
...
-- relist_property_for_marketing — نفس إصلاح enum في فلتر العروض
...
-- خطة تجريبية لحساب «فرد» (individual) إن لم تُدرَج سابقاً
...
SELECT
  N'تجريبي ٣ أيام', 'Trial', 'individual',
  0::numeric, 0::numeric, 0, 1, 1, NULL::int,
  true, true, false, false, 0::numeric(5,2), 0, true, true
WHERE NOT EXISTS (
  SELECT 1 FROM public.subscription_plans p
  WHERE p.user_type = 'individual' AND p.is_trial_plan = true
);
