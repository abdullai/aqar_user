-- Re-deploy create_listing_contract_from_offer with valid contract_status values.
-- Older deployments may still try to insert status = 'contract', which is not
-- part of the contract_status enum.
...
BEGIN
  ...
  IF req.owner_id IS DISTINCT FROM uid AND off.marketer_id IS DISTINCT FROM uid THEN
  ...
  IF v_existing IS NOT NULL THEN
    SELECT c.status::text INTO v_st
    FROM public.listing_contracts c
