-- =============================================================================
-- تحليل السوق: لقطة موحّدة (أرقام + ترتيب) — SECURITY DEFINER
-- نفّذ في Supabase SQL Editor إن لم تكن الدالة موجودة بعد.
-- =============================================================================
...
BEGIN
  ...
  IF to_regclass('public.org_units') IS NOT NULL THEN
    BEGIN
      ...
      org_listing AS (
        SELECT
          ou.id AS org_id,
          COALESCE(ou.account_type::text, 'org') AS org_kind,
          ...
          COALESCE(
            ...
            COALESCE(ou.account_type::text, 'org')
          ) AS label
  ...
  RETURN jsonb_build_object(
    ...
    'listings', COALESCE(listings_obj, jsonb_build_object(
      'published_total', 0,
      'new_last_7d', 0,
      'new_last_30d', 0,
      'featured_total', 0,
      'by_account_type', '{}'::jsonb,
      'by_property_type', '{}'::jsonb
    )),
    'listing_requests', jsonb_build_object(
      'total', lr_total,
      'by_status', COALESCE(lr_by_status, '{}'::jsonb)
    ),
...
GRANT EXECUTE ON FUNCTION public.get_market_insights_snapshot()
