-- =============================================================================
-- شغّل هذا الملف كاملاً من القرص (لا تلصق نصاً مقطوعاً).
-- محرر Supabase قطع السكربت عند IF v_has_pending THEN وأضاف RLS خاطئاً.
-- استخدم فاصل $owner_return$ بدل $$ لتفادي أخطاء الاقتباس.
-- =============================================================================

-- جزء 1: العمود فقط (يكفي غالباً لإيقاف خطأ round_no فوراً)
...
-- جزء 2: تعريف الدالة كاملاً (يجب أن ينتهي بـ $owner_return$ ثم GRANT)
...
DECLARE
  ...
  v_invites_has_round boolean := false;
BEGIN
  ...
  IF to_regclass('public.listing_request_invites') IS NOT NULL THEN
    SELECT EXISTS (
      ...
      WHERE c.table_schema = 'public'
        AND c.table_name = 'listing_request_invites'
        AND c.column_name = 'round_no'
    ) INTO v_invites_has_round;

    IF v_invites_has_round THEN
      UPDATE public.listing_request_invites inv
         SET status = 'pending',
             round_no = v_round,
             updated_at = now()
